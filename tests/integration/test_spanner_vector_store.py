...
from langchain_google_spanner.vector_store import SpannerVectorStore, TableColumn, SecondaryIndex, QueryParameters, DistanceStrategy
...
class TestSpannerVectorStoreGoogleSQL: 
    ...
    def setup_database(self, client):
        SpannerVectorStore.init_vector_store_table(
            ...
            table_name= table_name,
            id_column= "row_id",
            metadata_columns=[TableColumn(name="metadata", type="JSON", is_null=True), TableColumn(name="title",  type="STRING(MAX)", is_null=False)],
            secondary_indexes=[SecondaryIndex(index_name="test_index_1", columns=["row_id"])])
    ...
    def test_spanner_vector_add_data1(self, setup_database):
        ...
        db = SpannerVectorStore(instance_id= instance,
                database_id= google_database,
                table_name= table_name,  id_column= "row_id", ignore_metadata_columns=[], embedding_service=embeddings, metadata_json_column="metadata")
    ...
    def test_spanner_vector_add_data2(self, setup_database):
        ...
        texts = ["Langchain Test Text 1", "Langchain Test Text 2", "Langchain Test Text 3"]
        ids = [str(uuid.uuid4()) for _ in range(len(texts))]
        ids_row_inserted = db.add_texts(texts=texts, ids=ids, metadatas=[{"title": "Title 1"}, {"title": "Title 2"}, {"title": "Title 3"}]
                                        )
        assert ids == ids_row_inserted
    ...
    def test_spanner_vector_search_data1(self, setup_database):
        ...
        docs = db.similarity_search("Testing the langchain integration with spanner", k = 2)
    ...
    def test_spanner_vector_search_data2(self, setup_database):
        ...
        embeds = embeddings.embed_query("Testing the langchain integration with spanner") 
    ...
    def test_spanner_vector_search_data3(self, setup_database):
        ...
        db = SpannerVectorStore(instance_id= instance,
                        database_id= google_database,
                        table_name= table_name,  id_column= "row_id", ignore_metadata_columns=[], embedding_service=embeddings, metadata_json_column="metadata",
                        query_parameters= QueryParameters(distance_strategy=DistanceStrategy.COSINE,  max_staleness = datetime.timedelta(seconds=15)))
        
        docs = db.similarity_search("Testing the langchain integration with spanner", k = 3)
    ...
    def test_spanner_vector_search_data3(self, setup_database):
        ...
        docs = db.max_marginal_relevance_search("Testing the langchain integration with spanner", k = 3)
...
class TestSpannerVectorStorePGSQL: 
    ...
    def setup_database(self, client):
        SpannerVectorStore.init_vector_store_table(
            ...
            table_name= table_name,
            id_column= "row_id",
            metadata_columns=[TableColumn(name="metadata", type="JSONB", is_null=True), TableColumn(name="title",  type="TEXT", is_null=False)],
            secondary_indexes=[SecondaryIndex(index_name="test_index_1", columns=["row_id"])])
